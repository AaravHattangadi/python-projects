...
if (year % 4) == 0: # checks if the year is divisible by 4; If yes, it continues; If no, it jumps to line 22-23
    if (year % 100) == 0: # Checks if the year is divisble by 100; If yes, it continues; if no, it jumps to line 20-21
        if (year % 400) == 0: # Checks if the year is divisible by 400; If yes, it continues; If no, it jumpes to line 18-19
            print("{0} is a leap year".format(year)) # Prints argument 0 {0}, that is the var 'year' followed by the string, is a leap year. This argument is formatted into a preset year format by the command ".format(year)"
        else:
            print("{0} is not a leap year".format(year)) # Prints argument 0 {0}, that is the var 'year' followed by the string, is not a leap year. This argument is  formatted into a preset year format by the command ".format(year)"
    else:
